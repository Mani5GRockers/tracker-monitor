<?php
// index.php with pagination support

date_default_timezone_set('Asia/Kolkata');
define('DATA_FILE', 'trackers_data.json');
define('ITEMS_PER_PAGE', isset($_GET['limit']) ? max(10, intval($_GET['limit'])) : 20);
define('CURRENT_PAGE', isset($_GET['page']) ? max(1, intval($_GET['page'])) : 1);
...
$totalItems = count($trackers);
$totalPages = ceil($totalItems / ITEMS_PER_PAGE);
$trackers = array_slice($trackers, (CURRENT_PAGE - 1) * ITEMS_PER_PAGE, ITEMS_PER_PAGE, true);
...
$totalTrackers = count($trackers);
$totalPages = ceil($totalTrackers / $limit);
$offset = ($page - 1) * $limit;

// Slice trackers for current page
$pagedTrackers = array_slice($trackers, $offset, $limit, true);
...
function renderPagination($current, $total, $limit) {
    ...
    // Limit selector
    $html .= '<form method="GET" style="display:inline-block; margin-right:10px;">
    ...
    if ($total > 1) {
        for ($i = 1; $i <= $total; $i++) {
            $style = $i == $current ? "font-weight:bold; color:#007bff;" : "";
            $html .= "<a href=\"?page=$i&limit=$limit\" style=\"margin:0 5px; $style\">$i</a>";
        }
    }
...
<?= renderPagination($page, $totalPages, $limit); ?>
